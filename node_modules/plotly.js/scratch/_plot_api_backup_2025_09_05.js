...
function sketchifyFunc(gd) {
    function sketchify() {
        console.log('calling sketchify');
        // Get value of `sketchmode` from the layout
        var sketchmode = gd._fullLayout.sketchmode;
        if(!sketchmode) {
            console.log('sketchmode is false');
            return;
        }
        console.log('sketchmode is true');

        // Get the element with id 'main-svg' which is a child (several levels deep) of gd
        const mainSvg = d3.select(gd).select(".main-svg").node();
        ...
        d3.select(mainSvg).selectAll("path").each(function() {
            ...
            if(d) {
                ...
                const roughPathPath = d3.select(roughPath).select("path").node();
                // // Set attributes of roughPathPath to match those of the original path
                // for(const attr of path.node().attributes) {
                //     if(attr.name === "d") continue; // Don't overwrite the 'd' attribute
                //     roughPathPath.setAttribute(attr.name, attr.value);
                // }
                // console.log('roughPath: ', roughPath);
                // console.log('roughPathPath: ', roughPathPath);

                // // Append the rough path to the same parent
                // d3.select(path.node().parentNode).enter().append(roughPathPath);
                // path.remove()

                // set 'd' attribute of original path to that of rough path
                path.attr("d", roughPathPath.getAttribute("d"));
                path.attr("stroke", "null");
                path.attr("stroke-width", "1");
                path.attr("fill", "none");


                // path.node().parentNode.replaceChild(roughPathPath, path.node());
            }
        });
    }
    return sketchify;
