...
function appendBarText(gd, plotinfo, bar, cd, i, x0, x1, y0, y1, r, overhead, opts, makeOnCompleteCallback) {
    ...
    // Check for clipped text (both 'outside' and 'auto' that became outside)
    if(textPosition === 'outside' || textPosition === 'auto') {
        var textBBox = textSelection.node().getBoundingClientRect();
        var svgBBox = gd.getBoundingClientRect();

        var textLeft = textBBox.left - svgBBox.left;
        var textRight = textBBox.right - svgBBox.left;
        var textTop = textBBox.top - svgBBox.top;
        var textBottom = textBBox.bottom - svgBBox.top;

        var plotLeft = xa._offset;
        var plotRight = xa._offset + xa._length;
        var plotTop = ya._offset;
        var plotBottom = ya._offset + ya._length;

        if(textLeft < plotLeft || textRight > plotRight || textTop < plotTop || textBottom > plotBottom) {
            console.log('text is clipped:', text);
        }
        
        // Debug: compare actual rendered text dimensions
        console.log('DEBUG actual text dimensions:', text, {
            renderedWidth: textBBox.width,
            renderedHeight: textBBox.height,
            position: {left: textLeft, right: textRight, top: textTop, bottom: textBottom}
        });
    }
}
