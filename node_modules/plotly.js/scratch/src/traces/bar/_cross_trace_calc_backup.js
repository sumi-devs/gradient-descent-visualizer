...
function setBaseAndTop(sa, sieve) {
    ...
    for(var i = 0; i < calcTraces.length; i++) {
        ...
        addTextPaddingToExtremes(extremesOpts, fullTrace, sa);
...
// Helper function to check if trace has outside text
function hasTextOutside(trace) {
    console.log('hasTextOutside?');
    // Check if there's any text to display
    if(!trace.text || (Array.isArray(trace.text) && trace.text.every(function(t) { return !t; }))) {
        return false;
    }
    ...
    return Array.isArray(textpos) ? 
        textpos.some(function(pos) { return pos === 'outside' || pos === 'auto'; }) :
        (textpos === 'outside' || textpos === 'auto');
}

// Helper function to estimate maximum text height for outside text
function estimateMaxTextHeight(trace) {
    var font = trace.outsidetextfont || trace.textfont;
    if(!font || !font.size) return 0;
    ...
    return fontSize + TEXTPAD;
}

// Helper function to add text padding to extremes options
function addTextPaddingToExtremes(extremesOpts, fullTrace, sa) {
    if(!hasTextOutside(fullTrace)) return;
    
    var textHeightPixels = estimateMaxTextHeight(fullTrace);
    var isHorizontal = fullTrace.orientation === 'h';
    var axisIncreasing = sa.range && sa.range[1] > sa.range[0];

    if(isHorizontal) {
        extremesOpts[axisIncreasing ? 'ppadplus' : 'ppadminus'] = textHeightPixels;
    } else {
        // For vertical bars: y-axis has negative _m, so "up" is ppadminus
        extremesOpts[axisIncreasing ? 'ppadminus' : 'ppadplus'] = textHeightPixels;
    }
}
