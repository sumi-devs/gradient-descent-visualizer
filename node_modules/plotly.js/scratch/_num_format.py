...
def is_si_format(exponent_format):
    ...
    return exponent_format == 'SI' or exponent_format == 'B'


def beyond_si(exponent):
    """Check if exponent is beyond SI prefix range."""
    return exponent > 14 or exponent < -15
...
def num_separate(value, separators, separate_thousands=False):
    ...
    decimal_sep = separators[0] if len(separators) > 0 else '.'
...
def num_format(v: float, ax: Dict[str, Any], fmtoverride: Optional[str] = None, hover: bool = False) -> str:
    ...
    if hover:
        ah = {
            ...
            'range': ax.get('range', [0, 1]) if ax.get('showexponent') == 'none' else [0, v or 1]
        }
        auto_tick_round(ah)
        tick_round = (ah.get('_tickround') or 0) + 4
        exponent = ah.get('_tickexponent', 0)
    ...
    if abs(v) < epsilon:
        ...
        if tick_round == 0:
            # Case 1: No decimal places
            v = str(int(math.floor(v)))
        elif tick_round < 0:
            # Case 2: Negative rounding
            v = str(int(round(v)))
            # Truncate digits and pad with zeros
            v = v[:len(v) + tick_round]
            for i in range(tick_round, 0):
                v += '0'
        else:
            # Case 3: Positive rounding
            v = str(v)
            dp_pos = v.find('.') + 1
            if dp_pos > 0:
                v = v[:dp_pos + tick_round]
